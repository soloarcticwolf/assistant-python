...
speaker_embeddings = torch.tensor(embeddings_dataset[7306]["xvector"]).unsqueeze(
    0
)  # you can change the index to change speaker.
...
async def transcribe_audio(audio_data):
    try:
        ...
        with tempfile.NamedTemporaryFile(
            suffix=".webm", delete=False, dir=temp_dir
        ) as temp_audio_file:
...
async def get_llm_response(text):
    modified_text = "Think and answer the query after the colon. query: "
    try:
        inputs = tokenizer_llm(modified_text + text, return_tensors="pt").to(device)
...
async def text_to_speech(text):
    try:
        inputs = processor_tts(text=text.split("Answer:")[1], return_tensors="pt")
        ...
        scipy.io.wavfile.write(
            wav_buffer, rate=16000, data=speech_cpu
